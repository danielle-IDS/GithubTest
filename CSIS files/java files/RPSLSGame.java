...
public class RPSLSGame {
	 ...
	 */
	private int getMenuChoice(Scanner in)
	{
		int menuChoice = 0;
		do
		{
	 ...
	 * @param in
	 */
	private void player1Mode(TheRockPaperScissorsLizardSpock RPSLS, Scanner in)
	{
	 ...
	 * @param in
	 */
	private void player2Mode(TheRockPaperScissorsLizardSpock RPSLS, Scanner in)
	{
		System.out.print("Player-1, Your Move (Rock, Paper, Scissors, Lizard, Spock): ");
		in.skip("\n");
	...
	public static void main(String[] args) {
		...
		int gameType = game.getMenuChoice(in);
		...
		while (gameType != QUIT)
		{
			...
			{				
				...
				{
					game.player1Mode(RPSLS, in);
				}
				else
				{
					game.player2Mode(RPSLS, in);
				}
				tallyCount[RPSLS.determineWinner()]++;
			}
			if(gameType == ONE_PLAYER)
			{
				System.out.println("Game Results: Player 1: " + tallyCount[P1_COUNT] + ", Computer: "+ tallyCount[P2_COUNT] + ", Ties: " + tallyCount[TIE_COUNT]);
			}
			...
			System.out.println();
			gameType = game.getMenuChoice(in);
